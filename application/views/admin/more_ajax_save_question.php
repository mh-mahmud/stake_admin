				...
				<form id="submit_new_question_form">
					<div class="card">
						<div class="card-header">
							<strong class="card-title">Bet Question Edit Form For: <?= $team_title;?> || Question Title : <?= $option_title ?></strong>
						</div>
						<div class="card-body">
						...
						<div class="card-body">
							...
							<?php
							$question_id_details = $this->db->query("SELECT * FROM `more_bet_question_option` WHERE `question_id` = '{$question_id}' GROUP BY question_title ORDER BY id ASC")->result();
							$i=1;
// dd($question_id_details);
							foreach ($question_id_details as $val):
								?>
								<table class="option_menu_<?= $i; ?>">
									...
									<tr>
										...
										<th colspan="2"><input type="text" class="form-control" id="question_<?= $i; ?>" name="question[<?= $i; ?>]" value="<?= $val->question_title;	 ?>" placeholder="type question here"></th>
