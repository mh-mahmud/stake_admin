			...
			<div class="col-md-10">
				<form id="submit_new_question_form">
					<div class="card">
						...
						<div class="card-body">
							<input type="hidden" value="<?= $bet_question_id; ?>" name="question_old_id">
							...
							<div class="form-group">
								<label for="sportsname">Title</label>
								<input type="text" name="option_title" id="option_title" value="<?= $option_title ?>" class="form-control">
							</div>
						...
						<div class="card-body">
							...
							<?php
							$sports_q = $this->db->query("SELECT * FROM `more_bet_question_option` WHERE question_id = '{$bet_question_id}' GROUP BY question_title ORDER BY id ASC")->result();
							$i=1;
							foreach ($sports_q as $val):
								...
								<table class="option_menu_<?= $i; ?>">
									...
									<tr>
										...
										<th colspan="2"><input type="text" class="form-control" id="question" name="question[<?= $i; ?>]" value="<?= $val->question_title;	 ?>" placeholder="type question here"></th>
										<th><input type="number" class="form-control" id="question_serial_<?= $i; ?>" name="question_serial[<?= $i; ?>]" value="<?= $val->question_title_serial;	 ?>"></th>
									...
									<tbody>
									<?php
									$q_op = $this->db->query("SELECT * FROM `more_bet_question_option` WHERE question_title = '{$val->question_title}' ORDER BY id ASC")->result();
...
<script>
	$('#submit_new_question_form').submit(function (e) {
		...
		$.ajax({
			type: 'POST',
			url: '<?= base_url('morequestion/edit_bet_question')?>',
			data: $(this).serialize(),
			dataType: 'json',
		}).done(function (data) {
