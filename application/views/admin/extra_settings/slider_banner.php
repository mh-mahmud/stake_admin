						...
						<table id="bootstrap-data-table" class="table table-striped table-bordered">
							...
							<?php $i=1; foreach ($slider as $val) : ?>
								<tr>
									<td><?= $val->serial; ?></td>
									<td>
										<img width="150" src="<?php echo base_url(); ?>assets/img/slider/<?php echo $val->file_name; ?>">
									</td>
									<td>
										<?php
										if ($val->status == 1) {
									...
									<td>
										<a href="javascript:void(0);" onclick="return sportssliderStChange(<?= $val->id; ?>,<?= $val->status==1?0:1; ?>);"><i class="fa fa-check-square"></i></a>
										&nbsp;&nbsp;&nbsp;
										<a href="javascript:void(0);" onclick="return deletesliderSports(<?= $val->id; ?>);"><i style="color: red" class="fa fa-close"></i></a>
									</td>
								</tr>
								<?php $i++; endforeach; ?>
							</tbody>
...
<script>
	$(document).ready(function(){
		$('#upload_form').on('submit', function(e){
			...
			{
				$.ajax({
					url:"<?php echo base_url(); ?>extrasettings/sports_slider_upload",
					method:"POST",
	...
	function sportssliderStChange(id,status) {
		swal(
			{
				title: "Confirm",
				type: "warning",
				...
				if (isConfirm) {
					$.ajax({
						type: 'POST',
						url: '<?php echo base_url()?>extrasettings/sports_slider_status_change',
						data: {
							id: id, status: status
						},
						dataType: 'json'
	...
	function deletesliderSports(id) {
		swal(
			{
				title: "Confirm",
				type: "warning",
				...
				if (isConfirm) {
					$.ajax({
						type: 'POST',
						url: '<?php echo base_url()?>extrasettings/sports_slider_delete',
						data: {
							id: id
						},
						dataType: 'json'
						...
						.done(function (data) {
							if (data.st==200){
								...
								window.location='<?= base_url("slider_banner") ?>';
							}
